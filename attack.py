...
class PGDAttack(robustml.attack.Attack):
    def __init__(self, sess, model, epsilon, max_steps=100, learning_rate=0.01, debug=False):
        ...
        one_hot = tf.expand_dims(tf.one_hot(self._label, 10), axis=0)
        # Changed tf.nn.softmax_cross_entropy_with_logits_v2
        # To tf.nn.softmax_cross_entropy_with_logits
        self._loss = tf.nn.softmax_cross_entropy_with_logits(logits=model.logits, labels=one_hot)
...
class FGSMAttack(robustml.attack.Attack):
